...
class BladeRFDriver:
    ...
    # ------------------------------------------------------------------
    # Autonomous FPGA sweep
    #
    # The Nios keeps the retune queue topped up on its own once started, so
    # the host issues no per-step retune and waits for no acknowledgement.
    # See pkt_retune2.c in the FPGA firmware.
    #
    # Commands ride in the spare bits of the control register, which
    # config_gpio_write() already reaches. Bits 29:22 and 6:0 are not decoded
    # by the fabric, so writing them disturbs nothing.
    #
    #   [29:22] payload byte
    #   [6:4]   opcode
    #   [3]     strobe -- the firmware acts only when this CHANGES, so a
    #           read-modify-write of unrelated bits never re-triggers a command
    # ------------------------------------------------------------------

    _SWEEP_MASK   = (0xff << 22) | (0x07 << 4) | (1 << 3)
    _SWEEP_DWELL  = 1
    _SWEEP_N_LO   = 2
    _SWEEP_N_HI   = 3
    _SWEEP_FIRST  = 4
    ...
    def _sweep_cmd(self, opcode, payload=0):
        """Issue one sweep command, toggling the strobe so the firmware sees it."""
        base = self.device.config_gpio_read() & ~self._SWEEP_MASK
        self._sweep_strobe = 1 - getattr(self, '_sweep_strobe', 0)
        word = (base
                | ((payload & 0xff) << 22)
                | ((opcode & 0x07) << 4)
                | (self._sweep_strobe << 3))
        self.device.config_gpio_write(word)

    def sweep_configure(self, first_profile, n_profiles, dwell_samples):
        """Set the sweep parameters. Always stops a running sweep."""
        self._sweep_cmd(self._SWEEP_STOP)

        # dwell goes least-significant byte first; the firmware shifts each in
        for shift in (0, 8, 16, 24):
            self._sweep_cmd(self._SWEEP_DWELL, (dwell_samples >> shift) & 0xff)

        self._sweep_cmd(self._SWEEP_N_LO,  n_profiles & 0xff)
        self._sweep_cmd(self._SWEEP_N_HI, (n_profiles >> 8) & 0xff)
        self._sweep_cmd(self._SWEEP_FIRST, first_profile & 0xff)

    def sweep_start(self, lead_blocks=4):
        """Begin sweeping.

        The firmware reads the sample counter itself and starts
        lead_blocks * 4096 samples from now, so no timestamp has to cross the
        command port. The lead gives it time to load the first profiles.
        """
        self._sweep_cmd(self._SWEEP_START, lead_blocks & 0xff)
