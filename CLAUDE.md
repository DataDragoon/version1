...
- LiDAR: **TF40-S** (UART, **9600 baud, Modbus RTU**, slave addr 1) on `/dev/ttyAMA3`
  (GPIO8/9, physical pins 24/21). Driver `pi/sensors/tf40s.py`. **Replaced the TF-LC02 on
  2026-09-11** — `tflc02.py` is deleted; do not resurrect it, the protocol and baud are both
  different. It is a **5-pin** part: `EN_PWR` must be held high or it answers nothing (here it
  is shorted to 3.3 V). 0.05-40 m, 1 mm resolution, ±2 mm, **5 Hz**. See the TF40-S section
  below for the command set and the register-read-is-a-command trap.
  UART0 (`/dev/serial0` → `ttyAMA10`) has a **dead RX peripheral** and must not be used; the
  `config.txt` comment claiming the overlay "creates /dev/ttyAMA0" is wrong for the Pi 5.
...
## LiDAR swap: TF-LC02 -> TF40-S (2026-09-11). WORKING and integrated as the main LiDAR.

New module on the same UART3 wiring. **It shares nothing with the TF-LC02 but the pins** --
different baud, different protocol, and one extra wire that is currently the blocker.

| | TF-LC02 (old) | **TF40-S (new)** |
|---|---|---|
| baud | 115200 | **9600** |
| protocol | custom `55 AA ...` framing | **Modbus RTU**, slave addr **1**, func **0x03** only |
| range | short | **0.05-40 m** (@90% refl; 0.05-20 m @10%) |
| resolution / accuracy | 1 mm / -- | 1 mm / **+-2 mm**, repeatability 1sigma <2 mm |
| update rate | 11-17 Hz (adaptive) | **5 Hz fixed** |
| wires | 4 (VCC/GND/TX/RX) | **5 -- adds EN_PWR** |
| supply | 3.3 V | 3.3 V, **NOT 5 V tolerant** (signals must be 3.3 V too) |

Manual: **Benewake BP-UM-TF40-S A00 (2024)**, not kept in the repo. Everything this project
needs from it is transcribed below -- pinout, the full command table, the reply format and the
error codes -- so the PDF should not be needed again; re-download it from Benewake if it is.
Note there is no `pdftotext`/poppler on this Pi, but `pip3 install --break-system-packages
pypdf` (already installed) extracts it fine.

**`EN_PWR` (pin 5) must be high for the module to run at all** -- "TF40-S does not work on
power by default", EN low = low-power mode. On this bench **EN is already shorted to 3.3 V and
pulled high (confirmed by the operator 2026-09-11), so EN is NOT the fault.** An earlier draft
of this section called a floating EN the blocker; that was wrong and is corrected here. Note
EN being hard-tied also means the module is always powered and always drawing its ~180 mA.

**RESOLVED 2026-09-11: the module's slave ID had become 0, the Modbus BROADCAST
address.** It presented as a completely dead sensor -- silent to every request, at
every baud, and a power cycle did not help. The cause is nasty because a device on
address 0 **executes** what it is sent but, per Modbus spec, **never answers a
unicast request**. So: scanning addresses 1-247 finds nothing (it is not on any of
them), passive listening finds nothing (it only speaks when asked), and every
diagnostic points at dead hardware.

**What broke the deadlock: the module still ACTED.** Broadcasting the laser-on
command (`00 10 00 03 00 01 02 00 01 ...`) made the visible laser blink, which
proves in one step that the module is powered, alive, and receiving -- with no
reply frame needed. **Use the laser as the liveness test on this device**; it is
the only check that works when the return path is in doubt.

Confirmed independently before touching anything, by taking GPIO9 out of UART mode
and reading the raw line (`pinctrl set 9 ip pd` + lgpio at ~490 kHz):
- **with an internal PULL-DOWN the line still read HIGH 200/200** -- something
  external was driving it, so the TX lead was connected and the module's output
  driver was powered and idling correctly;
- **zero edges** while commanding, so it genuinely never transmitted.
That pair rules out both a broken lead and a dead Pi receiver without unplugging
anything, and it is the test to repeat before ever suspecting UART3's RX the way
UART0's had to be suspected.

**Fix:** `recover_address()` in `pi/sensors/tf40s.py` -- write the slave ID
register (`0x000D`) addressed to broadcast 0, so it lands whatever the current ID
is: `00 10 00 0D 00 01 02 00 01 6B 1D`. Verified: the reply came back stamped
address `01`, and unicast reads at address 1 have worked since.

**Probable cause, and the lesson:** before the manual arrived, probing sent two
speculative *Benewake-family* `5A ...` frames (TF40-S is a Benewake product) and a
blind `func 0x03` sweep of registers `0x00-0xFF`. Per the gotcha below, **on this
device a register read IS a command**, and `0x000D` is the slave ID. Do not probe
this module with anything but documented frames.

**WORKING END TO END (2026-09-11).** At a tape-measured 1.000 m the driver reads
**mean 1000.6 mm, sd 0.49 mm** over 14/15 frames -- inside the +-2 mm spec and
quieter than the TF-LC02's 0.66-0.78 mm raw sigma ever was. `stream.py` publishes
it at a true **5.5 Hz**.

Getting there turned up two decode traps, and the first one is the important one:

**A VALID READING BEGINS WITH 0xFF, BECAUSE IT IS SIGN EXTENSION -- NOT AN ERROR.**
The module returns a SIGNED 32-bit distance carrying a large fixed negative
offset, so 1.000 m arrives as `-12501` = `0xFFFFCF2B`. The manual's error codes
are the EXACT values `0xFF000000` / `0xFE000000` / `0xFD000000` / `0xFC000000`,
and an early version of the driver tested `value >> 24` against those top bytes
instead -- so it threw away **every good measurement** as "miscalculation,
remeasure". That presented as a sensor communicating flawlessly (correct CRCs,
correct acks, steady 5.8 Hz) while being unable to range at anything, at any
distance, which sent the diagnosis chasing lenses and minimum range. **Compare
the whole 32-bit word, never the top byte.** The genuine `0xFF000000` was
eventually seen live -- thrown while the module was being moved -- which is what
proved the two are different things.

**`RANGE_OFFSET_MM = 13501` is EMPIRICAL and undocumented.** Nothing in the
manual mentions an offset and it carries no calibration command, so where it
comes from is unknown; it may be a corrupted internal zero, plausibly from the
same blind register sweep that moved the slave ID. Slope is taken as exactly
1 count = 1 mm, which is the datasheet *resolution* rather than an assumption.
**Re-derive it after any module swap or reset**: park at a carefully measured
distance D and read `raw` (the driver's `__main__` self-test prints it beside
every reading), then `RANGE_OFFSET_MM = D_mm - raw`.

**The driver runs the module in CONTINUOUS mode, and that is a measured choice.**
Single-shot polling answers only **16-52%** of requests for ~0.5-1.1 readings/s at
any pacing -- the module drops a request that arrives mid-measurement -- while
continuous gives **100% at 5.1-5.8 Hz**. Two consequences:
- `read_distance()` blocks on the stream, so `stream.py`'s poll loop SELF-PACES at
  the sensor's own cadence. `LIDAR_POLL_HZ = 0` (uncapped) is therefore correct
  here, not lazy; a cap could only add latency.
- **`lidar_seq` counts streamed frames directly** rather than using the
  TF-LC02-era value-change rule. One frame IS one measurement here, whereas a
  polled re-read could return a cached value. The old rule UNDER-counted badly on
  a static target -- measured at a fixed 1.000 m it reported ~2 Hz (the republish
  timeout) against a real 5.5 Hz, which would have made `lidar_n` and `lidar_std`
  describe fewer looks than were actually taken. `stream.py` branches on
  `TF40S.continuous` and keeps the old rule for the polled path.

Continuous mode **persists in the module across host restarts**, so the driver
stops it in `__init__` as well as `close()` -- a process killed without cleanup
otherwise hands the next one a stream it did not start.

**A `start continuous` issued while the module is still digesting the previous
`stop` is silently DROPPED, and the module then never streams.** Found on the
bench: running the driver's self-test and starting `stream.py` immediately after
gave **0 readings in 9 s**, publishing nulls with no error anywhere, while the
same driver standalone a moment later was perfect -- i.e. it does NOT recover on
its own, so this is a hang-forever failure and not a slow start. The driver now
re-arms the stream after `STREAM_RESTART_AFTER_S = 1.5` of silence; re-sending is
...
and must not treat an early None as a dead sensor.

**UART3 itself is fully healthy and is NOT the old dead-UART0 story.** Worth stating because
the opening symptom -- total silence, active and passive, at every baud -- is byte-for-byte
the signature of the dead RP1 UART0 receiver documented above. It is not that: `ttyAMA3`
inits clean, GPIO8/9 mux as TXD3/RXD3, TX raises real IRQs, and **RX demonstrably works,
because the module's valid replies came back through it.** No loopback needed.

### Pinout and commands (verified: our CRC16 reproduces all five published frames byte-for-byte)

Pins: 1 VCC 3.3V, 2 GND, 3 TTL_TXD, 4 TTL_RXD, **5 EN_PWR**.

| purpose | frame |
|---|---|
| **read distance / single measurement** | `01 03 00 0F 00 02 F4 08` |
| continuous measurement (5 Hz) | `01 03 00 01 00 02 95 CB` |
| stop continuous | `01 03 00 0A 00 02 E4 09` |
| laser on (for alignment) | `01 10 00 03 00 01 02 00 01 67 A3` |
| laser off | `01 10 00 03 00 01 02 00 00 A6 63` |
| baud -> 115200 | `01 10 00 00 00 02 04 00 01 C2 00 F3 0F` |
| change slave ID | `01 10 00 0D 00 01 02 <IH> <IL> <CL> <CH>` |

Wake response when EN goes high: `01 03 02 00 00 B8 44`.

Reply to a distance read is `01 03 04 <4 data bytes> <CRC>`. Those 4 bytes are ONE 32-bit
value: a distance in **mm**, unless the top byte is a sentinel --
`0xFF......` remeasure, `0xFE......` weak return / too slow, `0xFD......` target too
reflective, `0xFC......` out of range. **Check the top byte before trusting the value**, the
same trap as the TF-LC02's `error_code`/`8888` sentinel that shipped a spurious 8.888 m
standoff. Modbus exception replies are separate (`01 83 <code>`): 1 function, 2 start address,
3 register count, 4 register value.

**GOTCHA: on this device a register READ is a COMMAND.** The documented commands are all
`func 0x03` reads of magic registers -- 0x0F reads distance, 0x01 starts continuous, 0x0A
stops it, 0x0D is the slave ID. So a naive `0x00..0xFF` register sweep *issues mode changes as
it goes* (ours started and stopped continuous mode mid-scan, which is part of why the device's
behaviour drifted during probing). Do not scan this device's register space blind, and do not
write to registers without the manual -- 0x0000 is the baud-rate register and 0x000D the slave
ID, so a stray write can strand the module at an unknown baud/address.

### Integration: DONE (2026-09-11)

`pi/sensors/tf40s.py` is the driver and `tflc02.py` is DELETED. `stream.py` and
`lidar_noise_char.py` both use TF40S; `LIDAR_POLL_HZ` is 20 (not 200 -- one Modbus
transaction is ~18 ms at 9600 baud and the sensor only measures at 5 Hz) and
`LIDAR_STABLE_REPUBLISH_S` is 0.5 (it MUST clear the sensor's measurement period or it
invents readings between real ones; 0.25 cleared the TF-LC02's ~87 ms but not this
sensor's 200 ms). `lidarAntennaOffsetMm` is **75 mm**, and its localStorage key is
versioned to `lidar_antenna_offset_mm_v2` so browsers carrying a TF-LC02-era value pick
the new one up. Two consequences still to think about:
- **5 Hz is 2-3x SLOWER than the TF-LC02's 11-17 Hz.** "BG model continuous capture" above
  interpolates the lidar track and its accuracy rests on bracket gaps of 60-90 ms; at 5 Hz
  brackets are ~200 ms, so `MAX_BRACKET_GAP_S = 0.25` is marginal and the wave-speed budget
  (per-pass granularity is `v * lidar_period`) roughly triples. Re-derive both, do not assume.
- **The 40 m range is irrelevant here and the +-2 mm repeatability is the number that matters**
  -- standoff feeds coherent background subtraction, where the measured cost is ~0.4 dB at
  sigma 0.43 mm. Spec 1sigma <2 mm is conservative, but characterise it with
  `pi/sensors/lidar_noise_char.py` at the real standoff before trusting it, and re-measure
  `LIDAR_ANTENNA_OFFSET_MM` -- it is per-mounting and this is a different-sized module.
